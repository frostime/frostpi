...
- The Extension Host searches only the active session working directory by invoking `fd` from PATH or Pi's managed bin directory. Missing `fd` is reported in the completion menu.
- Each query starts a fresh, bounded fd process; a newer query terminates the previous process. No workspace path catalog is retained.
- Searches include files and directories, respect ignore files by default, apply `files.exclude`, and additionally apply `search.exclude` unless disabled. `.git` and `node_modules` are always excluded.
