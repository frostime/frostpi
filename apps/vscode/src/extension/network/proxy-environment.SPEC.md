...
- `custom` writes configured HTTP, HTTPS, ALL, and NO_PROXY values. Bare `host:port` values are normalized to `http://host:port` before writing env; explicit schemes are preserved.
