...
- `custom` writes configured HTTP, HTTPS, ALL, and NO_PROXY values. Bare `host:port` values are accepted in Settings and the guided wizard and normalized to `http://host:port` before writing env; explicit schemes are preserved.
