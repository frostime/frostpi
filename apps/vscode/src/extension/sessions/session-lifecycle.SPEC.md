...
A resumed session becomes ready after Pi's startup state is available; loading prior messages is separate. History loads are serialized. Session files larger than 8 MiB are not loaded automatically because Pi returns `get_messages` as one potentially large JSONL record; the user may explicitly request loading from the session menu.
