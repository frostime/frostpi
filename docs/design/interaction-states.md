...
- **Message fork:** available only for a user message with a resolved Pi entry id while the selected session is idle and history is loaded. During replacement the Composer remains disabled except for a Cancel Fork primary action. Success retains the original stable session id as stopped, selects a new temporary fork id, and focuses the host-seeded selected message draft. Cancellation stops the child, restarts the original, and leaves logical sessions and drafts unchanged.
