...
- **Turn trace:** with `frostpi.conversation.collapseTurnTrace` (default on), a completed turn hides everything before the final response behind one "Worked · N steps · duration" header. Expanded rows stay flat siblings (same chrome as a running turn). A labeled "Reply" break — not a bare Markdown-like rule — separates the work trace from the final response; collapsed rows are not mounted. Running, aborted, and error turns stay flat. Expanding is local to that turn for the current Webview mount.
