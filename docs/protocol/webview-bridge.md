...
The host sends a complete snapshot after `ready` and whenever the active session changes. Stable-session updates replace scalar fields and transport ordered `turns` and `notices` through collection deltas. On `ready` and when one of the supported VS Code Chat typography settings changes, the host sends `setChatTypography`; the Webview applies the Chat message font size to rendered Markdown messages and code blocks, the Chat message font family to rendered Markdown messages, the Chat editor font family to the Composer and code blocks, and the Chat editor font size to the Composer.
