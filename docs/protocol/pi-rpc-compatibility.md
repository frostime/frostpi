...
Message-level Fork uses `get_entries` to bind the displayed user message to its stable Pi entry id, then calls `fork(entryId)`. Pi replaces the active runtime with a new session and returns the selected text for editing; FrostPi rebuilds the active projection and restores projected image attachments itself because the fork response contains text only.
